...
<?php } else { //*****************UPLOAD DL************************
    ...
    $api = new Instamojo\Instamojo(
        "1bf34c6da1ee6614445663e65365231a",
        "f7d8746ab010d43828996a63a79977cd",
...
function calc_price(
    ...
    for ($i = 0; $i < $no_of_loop; $i++) {
        ...
        ); // next day start is end of last date

        $daystart_time = date("Y-m-d 00:00:00", strtotime($strt_dt_tm)); // new day start time from 12.00 pm

        $totalhrperday =
            (strtotime($day_end_time) - strtotime($strt_dt_tm)) / 3600; // no of hour per day
